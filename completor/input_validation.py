...
def _check_for_errors(df_comp: pd.DataFrame, well_name: str, idx: int) -> None:
    ...
    if df_comp[Headers.DEVICE_TYPE].iloc[idx] not in Content.DEVICE_TYPES:
        raise CompletorError(
            ...
            "Valid types are PERF, AICD, ICD, VALVE, RCP, DUALRCP, and ICV."
        )
...
def set_format_wsegrcp(df_temp: pd.DataFrame) -> pd.DataFrame:
    """Format the well segments Rate Controlled Production (RCP) data.

    Args:
        df_temp: Well segments RCP device data.
    ...
    df_temp[columns] = df_temp[columns].astype(np.float64)
    # Create ID device column
    df_temp.insert(0, Headers.DEVICE_TYPE, np.full(df_temp.shape[0], Content.RATE_CONTROLLED_PRODUCTION))
    return df_temp


def set_format_wsegdualrcp(df_temp: pd.DataFrame) -> pd.DataFrame:
    """Format the well segments Dual Rate Controlled Production (DUALRCP) table.

    Args:
        df_temp: Well segments dual rate controlled production table.

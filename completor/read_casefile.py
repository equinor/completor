...
class ReadCasefile:
    ...
    def read_wsegdensity(self) -> None:
        ...
        if density_index_start == density_index_end and dar_index_start == dar_index_end:
            if (
                Content.DENSITY_BASED in self.completion_table[Headers.DEVICE_TYPE]
                or Content.DENSITY_ACTIVATED_RECOVERY in self.completion_table[Headers.DEVICE_TYPE]
            ):
                raise CompletorError(
                    f"{Keywords.DENSITY_DRIVEN} keyword must be defined, " "if DENSITY is used in the completion."
                )
        else:
            # Table headers
            header = [
    ...
    def read_wsegdualrcp(self) -> None:
        ...
        """
        dualrcp_index_start, dualrcp_index_end = parse.locate_keyword(self.content, Keywords.DUAL_RATE_CONTROLLED[0])
        aicv_index_start, aicv_index_end = parse.locate_keyword(self.content, Keywords.DUAL_RATE_CONTROLLED[1])

        # Determine which keyword is present
        if dualrcp_index_start == dualrcp_index_end and aicv_index_start == aicv_index_end:
            if (
                Content.DUAL_RCP in self.completion_table[Headers.DEVICE_TYPE]
                or Content.AUTONOMOUS_INFLOW_CONTROL_VALVE in self.completion_table[Headers.DEVICE_TYPE]
            ):
                raise CompletorError(
                    f"{Keywords.DUAL_RCP} keyword must be defined, " "if DUALRCP is used in the completion."
                )
        else:
            # Table headers
            header = [
            ...
            if not check_contents(device_checks, self.wsegdualrcp_table[Headers.DEVICE_NUMBER].to_numpy()):
                raise CompletorError(
                    f"Not all devices in COMPLETION are specified in {Keywords.DUAL_RATE_CONTROLLED[choice_index]}"
                )

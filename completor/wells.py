...
class Lateral:
    ...
    def _get_devices(df_completion: pd.DataFrame, df_well: pd.DataFrame, case: ReadCasefile) -> pd.DataFrame:
        ...
        if Content.DENSITY_BASED in active_devices:
            df_well = completion.get_device(df_well, case.wsegdensity_table, Content.DENSITY_BASED)
        if Content.DUAL_RCP in active_devices:
            df_well = completion.get_device(df_well, case.wsegdualrcp_table, Content.DUAL_RCP)

...
It is more practical to run from ERT to debug where ERT will give detailed messages on which point that it failed.

```
DEFINE <RMS_VERSION>        12.1.4
-- DEFINE <RMS_WF_NAME>        PRED_COMPLETOR --used to run completor inside RMS
DEFINE <RMS_WF_NAME>        WELL_PLANNING --used to reun completor in ERT
```

[//]: # (![ERT configuration workflow definition]&#40;../../static/img/fmu/ert_1.png&#41;)

[//]: # (```)

[//]: # (-- Run Completor with ERT)

[//]: # (-- Completor)

[//]: # (FORWARD_MODEL RUN_COMPLETOR&#40;<ARG1>=<RUNPATH>/ert/input/config/op5_gp_aicd.config, <ARG2>=eclipse/include_pred/schedule/drogon_pred.sch, <ARG3>=eclipse/include_pred/&#41;)
...
Completor will then be connected into ERT with a forward model called `run_completor`.
